...
class TreadmillTestMetode : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSelanjutnya.setOnClickListener {
            val rg_respon = findViewById<RadioGroup>(R.id.rg_respon)
            val radiopositif = findViewById<RadioButton>(R.id.Positif)
            val radionegatif = findViewById<RadioButton>(R.id.Negatif)
            //RadioButton
            val selectedRadioButtonId = rg_respon.checkedRadioButtonId
            val selectedRB = findViewById<RadioButton>(selectedRadioButtonId)
            val respon = selectedRB.text.toString()
...
//            jarakberjalan
    ...
    private fun calculateTotalCairanHF(beratBadan: String): BigDecimal? {
        ...
        val cairanHF = beratBadanBD.multiply(BigDecimal("0.7")).multiply(BigDecimal("30")).setScale(1,
    ...
    private fun calculateHRMBB(usia: String): BigDecimal?{
        ...
        val hrmbb = BigDecimal("164").subtract((usiaHR.multiply(BigDecimal(0.7)))).setScale(1,
            RoundingMode.HALF_UP)
...
//    Mets
    private fun calculatemets20(mets: String): BigDecimal {
        ...
        val mets20 = ((mets.subtract(BigDecimal("1")).multiply(BigDecimal("0.2"))).add(BigDecimal("1"))).setScale(1,
            RoundingMode.HALF_UP)
        return mets20
    }
    private fun calculatemets40(mets: String): BigDecimal {
        val mets = BigDecimal(mets)
        val mets40 = ((mets.subtract(BigDecimal("1")).multiply(BigDecimal("0.4"))).add(BigDecimal("1"))).setScale(1,
            RoundingMode.HALF_UP)
        return mets40
    }
    private fun calculatemets60(mets: String): BigDecimal {
        val mets = BigDecimal(mets)
        val mets60 = ((mets.subtract(BigDecimal("1")).multiply(BigDecimal("0.6"))).add(BigDecimal("1"))).setScale(1,
            RoundingMode.HALF_UP)
        return mets60
    }
    //      v02
    ...
    private fun calculatespeed(vo: String): BigDecimal {
        ...
        val speed = ((vo.subtract(BigDecimal("3.5"))).divide(BigDecimal("0.1"))).setScale(1,
            RoundingMode.HALF_UP)
        return speed
    }
